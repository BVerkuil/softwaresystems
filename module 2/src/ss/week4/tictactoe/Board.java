...
public class Board {
    ...
    public boolean hasRow(Mark m) {
		...
		while(j < DIM && !hrow) {
			hrow = true;
			for(int i = 0; i < DIM && hrow; i++) {
				hrow = (this.getField(j,i) == m);
			}
			j++;
		}
    	return hrow;
    ...
    public boolean hasColumn(Mark m) {
    	...
    	int j = 0;
    		while(j < DIM && !hcol) {
    			hcol = true;
    			for(int i = 0; i < DIM && hcol; i++) {
    				hcol = (this.getField(i,j) == m);
    			}
    			j++;
    	}
    	return hcol;
    ...
    public boolean hasDiagonal(Mark m) {
    	boolean diagup = true;
    	boolean diagdown = true;
    	for(int i = 0; i < DIM && (diagup || diagdown); i++) {
    		diagup = diagup && getField(i, DIM-i-1) == m;
    		diagdown = diagdown && getField(i,i) == m;
    	}
	...
	private String newtoSting(int i, String row) {
		for (int j = 0; j < DIM; j++) {
